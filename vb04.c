...
void affStats();
int teleValide(char telephone[LONGUEUR_TEL + 1]);  
...
int dateValide(struct Date date) {
    ...
    if (date.mois == 1 || date.mois == 3 || date.mois == 5 || date.mois == 7 || date.mois == 8 || date.mois == 10 || date.mois == 12) {
        ...
        if (date.jour > 28) {
            return 0; // Février (non bissextile)
        }
    }
...
void modifReservation() {
    ...
    if (id < 1 || id > nombreReservations) {
        printf("Reservation non trouvee.\n");
        return;
    }

    Reservation *reservation = &reservations[id - 1];

    printf("Modifier le nom (ancien : %s) : ", reservation->nom);
    fgets(reservation->nom, sizeof(reservation->nom), stdin);
    reservation->nom[strcspn(reservation->nom, "\n")] = 0;  

    do {
        printf("Modifier le telephone (ancien : %s) : ", reservation->telephone);
        fgets(reservation->telephone, sizeof(reservation->telephone), stdin);
        reservation->telephone[strcspn(reservation->telephone, "\n")] = 0;  
        if (!teleValide(reservation->telephone)) {
            printf("Numero de telephone invalide.\n");
        }
    } while (!teleValide(reservation->telephone));

    printf("Modifier l'age (ancien : %d) : ", reservation->age);
    scanf("%d", &reservation->age);

    printf("Modifier le statut (ancien : %s) : ", reservation->statut);
    scanf("%s", reservation->statut);

    struct Date nouvelleDate;
    do {
        printf("Modifier la date (ancien : %02d/%02d/%d) : ", reservation->date.jour, reservation->date.mois, reservation->date.annee);
        scanf("%d/%d/%d", &nouvelleDate.jour, &nouvelleDate.mois, &nouvelleDate.annee);
        if (!dateValide(nouvelleDate)) {
            printf("Date invalide.\n");
        }
    } while (!dateValide(nouvelleDate));
    reservation->date = nouvelleDate;
...
void affReservation() {
    ...
    Reservation reservation = reservations[id - 1];
    printf("ID: %d\nNom: %s\nTelephone: %s\nAge: %d\nStatut: %s\nDate: %02d/%02d/%d\n",
           reservation.id, reservation.nom, reservation.telephone, reservation.age, reservation.statut, reservation.date.jour, reservation.date.mois, reservation.date.annee);
}
...
void affStats() {
    ...
    printf("Total des reservations : %d\n", total);
    printf("Reservations confirmees : %d\n", confirmes);
    printf("Reservations en attente : %d\n", attends);
